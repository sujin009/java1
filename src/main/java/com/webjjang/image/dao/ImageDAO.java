...
//         pstmt = con.prepareStatement(LIST);
	...
	public Long getTotalRow(PageObject pageObject) throws Exception {
		...
		try {
			...
			// 4. 실행 객체 & 데이터 세팅
			// 전체 데이터 개수 쿼리인 경우 조건이 있으면 where을 붙여야 한다. - true
	...
	public int write(ImageVO vo) throws Exception {
		...
		try {
			...
			throw new Exception("예외 발생 : 이미지 등록 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	public int update(ImageVO vo) throws Exception {
		...
		try {
			...
			// 6. 표시 또는 담기
			if (result == 0) { // 글번호가 존재하지 않는다. -> 예외로 처리한다.
				throw new Exception("예외 발생 : 번호 or 비밀번호가 맞지 않습니다. 정보를 확인해 주세요.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
	...
	public int delete(ImageVO vo) throws Exception {
		...
		try {
			...
			if (result == 0) { // 글번호가 존재하지 않거나 비번 틀림. -> 예외로 처리한다.
				throw new Exception("예외 발생 : 번호 or 아이디가 맞지 않습니다. 정보를 확인해 주세요.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
	...
	// list(), getTotalRow()에서 사용한다. list는 where가 반드시 있다.
	// getTotalRow()에서는 where가 없다. 단, 검색이 있는 경우 where을 추가해야 한다.
	private String getSearch(PageObject pageObject, boolean isWhere) {
		// or like 연산자 일때는 where 1=0 -> false를 앞에다 둔다.
		String sql = ""; // false
		String key = pageObject.getKey();
		String word = pageObject.getWord();
		// key안에 t가 포함되어 있으면 title로 검색한다.
		// 찾는 대상은 word 안에 데이터가 포함되어있다.
	...
	private int setSearchData(PageObject pageObject, PreparedStatement pstmt, int idx) throws SQLException {
		// pstmt의 순서 번호로 사용. 먼저 1증가 하고 사용한다.
		String key = pageObject.getKey();
		String word = pageObject.getWord();
		if (word != null && !word.equals("")) {
			// key안에 t가 포함되어 있으면 title로 검색한다.
